...
function Overworld() {
  ...
  function handleIconClick(evt) {
    ...
    const thisLevel = worldState.levels.filter(level => level._id === evt.target.dataset.level);
        ...
        <Dialog open={open}>
          <Grid container direction="row">
            <Grid item xs container direction="column" spacing={1}>
              <img src={showLevel[0].icon} width="100%" margin="0 auto"></img>
            </Grid>
                ...
                <Grid item xs container direction="column" spacing={2}>
                  <Grid item xs>
                    ...
                    <Typography variant="body2" gutterBottom>
                      Difficulty: {showLevel[0].difficulty}
                    </Typography>
                    <Typography variant="body2" gutterBottom>
                      Max Points: {showLevel[0].score_points}
                    </Typography>
                  </Grid>
                </Grid>
              </Grid>
  ...
  return (
    <Grid container direction="column" justify="center" alignItems="center" style={styles.worldContainer} className="worldMap">
      
      {worldLevels.map(level => {
          return (
            <>
            <img src={level.icon} className="icons" data-level={level._id} onClick={handleIconClick}></img>
            </>
          )
        })}
